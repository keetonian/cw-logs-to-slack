...
STACK_NAME ?= aws-cw-logs-to-slack
...
deploy: package
	pipenv run sam deploy --template-file $(SAM_DIR)/packaged-template.yml --stack-name $(STACK_NAME) --capabilities CAPABILITY_IAM --parameter-overrides SlackUrl=${SLACK_URL} LogGroupName=${LOG_GROUP_NAME} FilterPattern=${FILTER_PATTERN}
